...
# Odczyt punktów osnowy 3D z pliku geodezyjnego (ZAMIANA OSI: OpenCV X=Wschód, Y=Północ, Z=H)
coords_3D_dict = {}
...
# --- 3. PRZYGOTOWANIE DANYCH Z UKŁADEM CENTRYCZNYM ---
# Powrót do float32
punkty3D_raw_control = np.array([coords_3D_dict[k] for k in control_keys], dtype=np.float32)
punkty2D_control = np.array([data_gcp[k] for k in control_keys], dtype=np.float32)

# WYZNACZENIE ŚRODKA CIĘŻKOŚCI (Sztuczka numeryczna)
centroid = np.mean(punkty3D_raw_control, axis=0)

# Redukcja punktów osnowy do środka ciężkości
punkty3D_centric_control = punkty3D_raw_control - centroid
...
C = -R.T @ tvec + centroid.reshape(3, 1)
...
def calculate_reprojection_errors(keys, is_control=True):
    # Powrót do float32
    pts3D_raw = np.array([coords_3D_dict[k] for k in keys], dtype=np.float32)
    pts2D_orig = np.array([data_gcp[k] for k in keys], dtype=np.float32)
    
    # Przejście do układu centrycznego wyznaczonego przez punkty CONTROL
    pts3D_centric = pts3D_raw - centroid
...
VECTOR_SCALE = 50 # Współczynnik powiększenia strzałek na wykresie
...
for i, k in enumerate(control_keys):
    ax.text(orig_cntrl[i, 0] + 40, orig_cntrl[i, 1] + 40, f"{err_cntrl[i]:.1f} px", 
...
for i, k in enumerate(check_keys):
    ax.text(orig_check[i, 0] + 40, orig_check[i, 1] + 40, f"{err_check[i]:.1f} px", 
